...
public class MoveLeftLeg : MonoBehaviour
{
    ...
    [SerializeField] private AudioSource attractionSoundThree;
    ...
    private GameObject _robotThree;
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        {
            Destroy(_robotThree.gameObject);
        }
    }
    ...
    void GrabObject()
    {
        ...
        if (!TryGetTouchDrag(out _currPosition, ref _currId) || _prevId != _currId || _robotThree == null)
        {
            if (_prevId != _currId)
            {
                Debug.Log("Word");
                _robotThree = HitObject(_currPosition);
                Debug.Log("robot: " + _robotThree);
            }
            else
            {
                _robotThree = null;
        ...
        _robotThree.transform.Translate(movement * movementSpeed * Time.deltaTime);
    }
    ...
    GameObject HitObject(Vector2 touchPosition)
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity))
        {
            ...
            if (hit.transform.name.Contains("leftLegParent"))
            {
                Debug.Log("YoYoMa");
                _holdObject = true;
                if (_holdObject)
